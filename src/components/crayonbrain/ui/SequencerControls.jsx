...
const SequencerControls = ({ isPlaying, isInitializing, onPlay, onClear }) => {
    return (
        <div className="flex flex-row items-center justify-between w-full mb-4 md:mb-6 lg:mb-8 px-2">
            <button
                onClick={onPlay}
                className={`btn shadow-md bg-base-200 rounded-lg ${isPlaying ? 'text-red-300' : 'text-cyan-200'} w-24 md:w-28 lg:w-32`}
            ...
            <button
                onClick={onClear}
                className="btn shadow-md bg-base-200 rounded-lg w-24 md:w-28 lg:w-32"
